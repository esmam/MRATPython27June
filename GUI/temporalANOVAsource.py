...
from stats.ANOVAtoEelbrain import ANOVA22BESASpatial

class Ui_Form(object):
    def setupUi(self, Form):
        Form.setObjectName("FormANOVA")
        Form.resize(730, 491)
    ...
    def RunAnalysis(self):
        ...
        self.Results = ANOVA22BESASpatial(root, condition1Name, condition2Name,startTime,endTime, presTim,PThresh,minTemporalCluster, numPerm, anlsysTail, ROI)
...
'''
        ...
        #data1 = {'ClusterID':['1','2','3'], 'n_sources':['4','5','6'], 'hemisphere':['7','8','9'],'Start Time':['7','8','9'],'End Time':['7','8','9'],'Duration':['7','8','9'],'P-value':['7','8','9']}

        line1 = cluster_table[0]
        lines =  cluster_table[1:]
        id = []
        sourcemin=[]
        sourcemax=[]
        tstart = []
        ...
        data = cluster_table
        for el in cluster_table._table[1:]:
            id.append(str(el[0]))
            sourcemin.append(str(el[1]))

            sourcemax.append(str(el[2]))
            tstart.append(str(el[3]))
            tstop.append(str(el[4]))
            duration.append(str(el[5]))
            v.append(str(el[6]))
            p.append(str(el[7]))
            sig.append(str(el[8]))
            effect.append(str(el[9]))


        duration1 = self.Results.get('duration')
        durations= []
        for _len_ in duration1:

            durations.append(str(int(_len_*1000)))

        data = {'1 ClusterID':id, '2 source min':sourcemin, '3 source max':sourcemax,'4 start Time':tstart, '5 end Time':tstop,'6 duration':duration,'7 v':v,'8 P-value': p,'9 Sig':sig,'10 effect':effect} #,'Duration':['7','8','9'],'P-value':['7','8','9']}


        a = cluster_table[0]
        self.table = MyTable(self.sc,data, len(durations),10)#,startx,endx,stepx,cond1Data,cond2Data)
        l.addWidget(self.sc)
        self.table.show()
        #dc = MyDynamicMplCanvas(self.main_widget, width=5, height=4, dpi=100)
        #MyStaticMplCanvas.compute_initial_figure(self.main_widget)
        MyStaticMplCanvas.compute_initial_figure1(self.sc,startx,endx,stepx,cond1Data,cond2Data)
        l.addWidget(self.sc)
    ...
    def retranslateUi(self, Form):
        ...
        self.Condition1Label.setText(_translate("Form", "Condition Factor :"))
